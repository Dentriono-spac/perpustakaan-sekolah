...
function bukaMenu(namaMenu) {
  ...
  // Sembunyikan semua halaman/menu
  document.querySelectorAll(".menu-page").forEach(function(el) {
  ...
  // Statistik hanya ditampilkan di menu utama
  const statistik = document.getElementById("statistik");

  if (statistik) {
    statistik.style.display = "none";
  }

  // Muat data sesuai menu
  if (
    namaMenu === "keterlambatan" &&
    typeof loadKeterlambatan === "function"
  ) {
    loadKeterlambatan();
  }

  if (
    namaMenu === "bukuDipinjam" &&
    typeof tampilkanHasilPinjaman === "function"
  ) {
    // Jangan otomatis mencari sebelum ID dimasukkan
  }
}
...
function kembaliMenu() {
  ...
  // Matikan kamera kalau masih aktif
  if (
